...
function loadStat() {
  try {
    const d = localStorage.getItem(LS_STAT);
    if (d) statMap = JSON.parse(d);
  } catch (_) {}
}
function _saveStat() {
  try {
    localStorage.setItem(LS_STAT, JSON.stringify(statMap));
  } catch (_) {}
}

// ── 마스킹 단계 ──────────────────────────
const LS_STAGE = 'sb-stage';

function loadStage() {
  try {
    const s = localStorage.getItem(LS_STAGE);
    if (s !== null) stage = parseInt(s, 10) || 0;
  } catch (_) {}
}
...
// ── 공개(reveal) 집합 ─────────────────────
function isRevealed(ref) {
  return revealed.has(ref);
}
...
function revealAll() {
  verses.forEach(v => revealed.add(v.ref));
}
function hideAll() {
  revealed = new Set();
  clearAllStage2Flips();
...
function savePos(i) {
  try {
    localStorage.setItem(LS_POS, i);
  } catch (_) {}
}
function loadPos() {
  try {
    return parseInt(localStorage.getItem(LS_POS)) || 1;
  } catch (_) {
    return 1;
  }
}
...
function setSize(i) {
  ...
  const el = document.querySelector('.verse-wrap') || document.querySelector('.card-body');
  if (el) el.style.fontSize = SIZES[sIdx];
  try {
    localStorage.setItem(LS_SIZE, SIZES[sIdx]);
  } catch (_) {}
}
